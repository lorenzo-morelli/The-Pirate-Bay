...
struct Texture {
	...
	void init(BaseProject *bp, const char * file, VkFormat Fmt);
...
void Model<Vert>::loadModelOBJ(std::string file) {
	...
	std::cout << "Building\n";	
//	std::cout << "Position " << VD->Position.hasIt << "," << VD->Position.offset << "\instances";
//	std::cout << "UV " << VD->UV.hasIt << "," << VD->UV.offset << "\instances";
//	std::cout << "Normal " << VD->Normal.hasIt << "," << VD->Normal.offset << "\instances";
...
void Texture::init(BaseProject *bp, const char *  file, VkFormat Fmt = VK_FORMAT_R8G8B8A8_SRGB) {
