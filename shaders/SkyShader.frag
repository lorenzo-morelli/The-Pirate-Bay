...
void main() {
    ...
    vec3 lightTest = vec3(0.5f, 1.0f, 0.0f);
    ...
    vec3 highColor = vec3(150.0f, 255.0f, 150.0f)/255.f + yellow;
    vec3 darkColor = vec3(80.0f, 130.0f, 80.0f)/255.0f + yellow;

    if (gubo.spot.x == 0) {

        vec3 diffuse = toonDiffuse(lightTest, norm, eyeDir, darkColor + yellow, highColor );
